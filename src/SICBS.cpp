...
void SICBS::findConflicts(const Solution& solution, vector<Conflict>& conflicts) const {
  ...
  for (int agent1_id = 0; agent1_id < env.num_of_robots; ++agent1_id) {
    for (int agent2_id = agent1_id + 1; agent2_id < env.num_of_robots; ++agent2_id) {
      ...
      const auto times = static_cast<int>(floor(min_path_time / env.velocities[agent1_id])) + 1;
      ...
      for (int time = 0; time < times; ++time) {
        ...
        if (agent1_theta != 0.0) {
          agent1_point = make_tuple(get<0>(prev_point1) + env.velocities[agent1_id] * cos(agent1_theta) * time,
                                    get<1>(prev_point1) + env.velocities[agent1_id] * sin(agent1_theta) * time);
        }
        if (agent2_theta != 0.0) {
          agent2_point = make_tuple(get<0>(prev_point2) + env.velocities[agent2_id] * cos(agent2_theta) * time,
                                    get<1>(prev_point2) + env.velocities[agent2_id] * sin(agent2_theta) * time);
        }
